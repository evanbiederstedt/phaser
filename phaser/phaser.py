...
def main():
	...
	parser.add_argument("--output", "--o", help="Out prefix",required = True)
	...
	if check_dependency("samtools") == False: 
		fatal_error("External dependency 'samtools' not installed.")
	if check_dependency("bgzip") == False: 
		fatal_error("External dependency 'bgzip' not installed.")
	if check_dependency("tabix") == False: 
		fatal_error("External dependency 'tabix' not installed.")
	if check_dependency("bedtools") == False: 
		fatal_error("External dependency 'bedtools' not installed.")
	...
	for line in stream_vcf:
		...
		if line.startswith("#") == False:
			...
			for item in contig_ban:
				if item in chr: 
					fatal_error("Character '%s' must not be present in contig name. Please change id separtor using --id_separator to a character not found in the contig names and try again."%(item))
	...
	out_stream = open(args.output + ".variant_connections.txt", "w")
	...
	for connection in pool_output:
		...
		if conflicting_config_p < args.cc_threshold:
			...
			if variant_a+":0" not in dict_allele_connections: 
				dict_allele_connections[variant_a+":0"] = set([])
			if variant_a+":1" not in dict_allele_connections: 
				dict_allele_connections[variant_a+":1"] = set([])
			if variant_b+":0" not in dict_allele_connections: 
				dict_allele_connections[variant_b+":0"] = set([])
			if variant_b+":1" not in dict_allele_connections: 
				dict_allele_connections[variant_b+":1"] = set([])
	...
	stream_out = open(args.output +".allelic_counts.txt","w")
	...
	for block in block_haplotypes:
		...
		for variant in block:
			...
			if variant in dict_variant_overlap[chr]: 
				variant_connections[variant] = dict_variant_overlap[chr][variant]
			if variant+":0" in dict_allele_connections: 
				allele_connections[variant+":0"] = dict_allele_connections[variant+":0"]
			if variant+":1" in dict_allele_connections: 
				allele_connections[variant+":1"] = dict_allele_connections[variant+":1"]
	...
	stream_out_ase = open(args.output + ".haplotypic_counts.txt","w")
	...
	stream_out = open(args.output + ".haplotypes.txt","w")
	...
	stream_out_allele_configs = open(args.output + ".allele_config.txt","w")
	...
	for block in final_haplotypes:
		...
		if args.output_network in variants:
			...
			stream_out_network = open(args.output + ".network.links.txt","w")
			...
			stream_out_network = open(args.output + ".network.nodes.txt","w")
		...
		for variant in overlapped_variants:
			...
			for other_variant in overlapped_variants:
				...
				if var_chr == other_var_chr and other_variant != variant:
					if var_chr not in dict_variant_overlap: 
						dict_variant_overlap[var_chr] = {}
					if variant not in dict_variant_overlap[var_chr]: 
						dict_variant_overlap[var_chr][variant] = []
		...
		if use_as_cutoff == False or int(fields[4]) >= as_cutoff:
			...
			if read_allele in dict_variant_reads[var_id]['alleles']:
				...
				if bam_index not in haplo_count_bam_exclude or len(haplo_count_bam_exclude) == 0:
					if bam_index not in dict_variant_reads[var_id]['haplo_reads'][allele_index]: 
						dict_variant_reads[var_id]['haplo_reads'][allele_index][bam_index] = []
...
def phase_block(input):
	...
	for remove in remove_connections:
		...
		for remove_index in remove:
			...
			if remove_keys[0]+":0" in remaining_hap_pool[remove_keys[1]+":0"]: 
				remaining_hap_pool[remove_keys[1]+":0"].remove(remove_keys[0]+":0")
			if remove_keys[0]+":1" in remaining_hap_pool[remove_keys[1]+":0"]: 
				remaining_hap_pool[remove_keys[1]+":0"].remove(remove_keys[0]+":1")
			if remove_keys[0]+":0" in remaining_hap_pool[remove_keys[1]+":1"]: 
				remaining_hap_pool[remove_keys[1]+":1"].remove(remove_keys[0]+":0")
			if remove_keys[0]+":1" in remaining_hap_pool[remove_keys[1]+":1"]: 
				remaining_hap_pool[remove_keys[1]+":1"].remove(remove_keys[0]+":1")

			if remove_keys[1]+":0" in remaining_hap_pool[remove_keys[0]+":0"]: 
				remaining_hap_pool[remove_keys[0]+":0"].remove(remove_keys[1]+":0")
			if remove_keys[1]+":1" in remaining_hap_pool[remove_keys[0]+":0"]: 
				remaining_hap_pool[remove_keys[0]+":0"].remove(remove_keys[1]+":1")
			if remove_keys[1]+":0" in remaining_hap_pool[remove_keys[0]+":1"]: 
				remaining_hap_pool[remove_keys[0]+":1"].remove(remove_keys[1]+":0")
			if remove_keys[1]+":1" in remaining_hap_pool[remove_keys[0]+":1"]: 
				remaining_hap_pool[remove_keys[0]+":1"].remove(remove_keys[1]+":1")
	...
	for variant in variants:
		if variant+":0" in largest_block or variant+":1" in largest_block:
			...
			for connected_var in allele_connections:
				if variant+":0" in allele_connections[connected_var]: 
					allele_connections[connected_var].remove(variant+":0")
				if variant+":1" in allele_connections[connected_var]: 
					allele_connections[connected_var].remove(variant+":1")
		else:
...
def write_vcf():
	...
	vcf_out = open(args.output + ".vcf","w")
			...
			if args.chr == "" or chrom == args.chr:
				if "GT" in vcf_columns[8]:
					...
					for tag in phaser_tags:
						if tag not in vcf_format_fields: 
							vcf_format_fields.append(tag)
					...
					if unique_id in set_phased_vars:
						...
						if "-" not in gw_phase_out:
							...
							if gw_stat >= args.gw_phase_vcf_min_confidence:
								if "|" in xfields[gt_index] and xfields[gt_index] != new_phase: 
									phase_corrections += 1
								if "/" in xfields[gt_index] and xfields[gt_index] != "./." and xfields[gt_index] != new_phase: 
									unphased_phased += 1
	...
	subprocess.check_call("set -euo pipefail && "+"bgzip -f "+args.output+".vcf; "+tabix_cmd+" -f -p vcf "+args.output+".vcf.gz", shell=True, executable='/bin/bash')
